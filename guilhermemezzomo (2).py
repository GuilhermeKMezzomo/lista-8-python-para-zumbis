...
def preposições(word):
    zumbi = tuple('zmb')
    outras = tuple('tshjpnwlrcxkqvdgf')

    if word.startswith(zumbi) and word.endswith(outras):
        return True
    return False
...
for palavra in txtB:
    if preposições(palavra):
        preposição.append(palavra)

print(len(preposição))
...
def verbo(word):
    ...
    if len(word) == 7 and word.endswith(outras):
        return True
    return False

def primeiraPessoa(word):
    outras = tuple('tshjpnwlrcxkqvdgf')

    if len(word) == 7 and word.endswith(outras) and word.startswith(outras):
        return True
    return False
...
for palavra in txtB:
    ...
    if primeiraPessoa(palavra):
        primeirapessoa.append(palavra)

print(len(verbos), len(primeirapessoa))
